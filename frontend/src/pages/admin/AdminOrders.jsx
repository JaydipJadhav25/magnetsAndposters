...
import { FiChevronDown, FiExternalLink, FiSearch, FiDownload } from 'react-icons/fi'
...
export default function AdminOrders() {
  ...
  const [trackForm, setTrackForm] = useState({ status:'', trackingNumber:'', courier:'', note:'' })
  ...
  const handleUpdateStatus = async (orderId) => {
    if (!trackForm.status) { toast.error('Select a status'); return }
    setUpdating(orderId)
    try {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <div className="relative flex-1 min-w-[220px]">
          ...
          <input
            type="text" placeholder="Search by order # or customer…"
      ...
      {loading ? (
        <div className="space-y-3">
          {[1,2,3,4].map(i => <div key={i} className="bg-white rounded-xl h-20 animate-pulse" />)}
        </div>
            ...
            <div key={order._id} className="bg-white rounded-xl shadow-sm overflow-hidden">
              ...
              >
                <div className="flex items-center gap-4">
                  <div>
                    <p className="font-mono font-semibold text-dark text-sm">{order.orderNumber}</p>
                  ...
                  <div className="hidden sm:block">
                    <p className="text-sm font-medium text-dark">{order.user?.name || order.guestEmail || 'Guest'}</p>
                    <p className="text-xs text-gray-400">{order.items.length} item(s)</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <span className="font-bold text-dark text-sm">{formatPrice(order.total)}</span>
                  <span className={`text-xs font-semibold px-2.5 py-1 rounded-full border capitalize ${STATUS_COLORS[order.orderStatus] || 'bg-gray-100 text-gray-500'}`}>
                    {order.orderStatus}
                  </span>
                  <FiChevronDown
                    size={16}
                    ...
                    <div>
                      ...
                      <div className="space-y-3">
                        {order.items.map((item, i) => (
                          <div key={i} className="flex gap-3 items-start bg-white rounded-lg p-3 border border-gray-100">
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <div className="bg-white rounded-lg p-3 border border-gray-100 text-sm text-gray-600">
                          ...
                          <p className="text-brand-600 mt-1">📱 {order.shippingAddress?.phone}</p>
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="bg-white rounded-lg p-3 border border-gray-100 space-y-2">
                          ...
                          <input type="text" placeholder="Tracking number (optional)"
                            ...
                            className="input-field text-sm py-2" />
                          <input type="text" placeholder="Courier name (optional)"
                          ...
                          <textarea rows={2} placeholder="Internal note (optional)"
                            ...
                            onChange={e => setTrackForm(f => ({...f, note: e.target.value}))}
                          ...
                          >
                            {updating === order._id ? 'Updating…' : 'Update Order'}
                          </button>
                        </div>
                      </div>
                    </div>
                      ...
                      <div className="flex gap-3 flex-wrap">
                        {order.statusHistory.map((h, i) => (
                          <div key={i} className="bg-white rounded-lg px-3 py-2 border border-gray-100 text-xs">
                            <p className="font-semibold text-dark capitalize">{h.status}</p>
                            <p className="text-gray-400">{new Date(h.timestamp).toLocaleDateString('en-IN')}</p>

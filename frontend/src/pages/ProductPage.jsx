...
import { FiShoppingCart, FiUpload, FiX, FiCheck, FiMinus, FiPlus } from 'react-icons/fi'
        ...
        <div className="space-y-6">
          <div>
            ...
            <div className="flex items-center gap-3 mt-3">
              <span className="text-2xl font-bold text-dark">{formatPrice(displayPrice)}</span>
              {displayMrp && displayMrp > displayPrice && (
                <>
                  <span className="text-lg text-gray-400 line-through">{formatPrice(displayMrp)}</span>
          ...
          {product.variants?.length > 0 && (
            <div>
              <p className="text-sm font-semibold text-dark mb-2">
                Size: <span className="text-brand-600">{selectedVariant?.size}</span>
              </p>
          ...
          {product.requiresCustomImage && (
            <div className="bg-brand-50 border border-brand-200 rounded-xl p-4">
              ...
              <p className="text-xs text-gray-500 mb-3">
                Upload a high-resolution image for best print quality. JPG, PNG, HEIC accepted (max 50MB).
              </p>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3 border border-gray-200 rounded-lg px-4 py-2">
              <button onClick={() => setQty(Math.max(1, qty - 1))} className="text-dark hover:text-brand-600"><FiMinus size={16} /></button>
              <span className="w-8 text-center font-medium">{qty}</span>
              <button onClick={() => setQty(qty + 1)} className="text-dark hover:text-brand-600"><FiPlus size={16} /></button>
            </div>
          </div>
          ...
          <div className="flex gap-3 pt-2">
            ...
            >
              ...
              {addingToCart ? 'Added!' : uploading ? 'Uploading...' : 'Add to Cart'}
            </button>
          </div>

          {/* Shipping note */}
          <p className="text-xs text-gray-400 text-center">
            🚚 Free shipping on orders above ₹699 · Delivered in 5–7 business days
          </p>
        </div>
      </div>
    </div>

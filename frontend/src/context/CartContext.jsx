...
const cartReducer = (state, action) => {
  switch (action.type) {
    ...
    case 'UPDATE_IMAGE': {
      const items = [...state.items]
      items[action.index] = { ...items[action.index], customImage: action.image }
      return { ...state, items }
    }
    case 'CLEAR':
      return { items: [] }
...
export function CartProvider({ children }) {
  ...
  const addItem    = (item) => { dispatch({ type: 'ADD', item }); setIsOpen(true) }
  const removeItem = (index) => dispatch({ type: 'REMOVE', index })
  const updateQty  = (index, qty) => dispatch({ type: 'UPDATE_QTY', index, qty })
  const updateImage = (index, image) => dispatch({ type: 'UPDATE_IMAGE', index, image })
  ...
  return (
    <CartContext.Provider value={{ items: state.items, subtotal, itemCount, isOpen, setIsOpen, addItem, removeItem, updateQty, updateImage, clearCart }}>
      {children}
